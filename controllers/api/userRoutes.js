...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user) {
      res.status(400).json({ message: "No user account found! 1" });
      return;
    }
    console.log(user);
    ...
    if (!validPassword) {
      res.status(400).json({ message: "No user account found! 2" });
      return;
    }
    ...
    res.status(400).json({ message: "No user account found! 3" });
  }
});
